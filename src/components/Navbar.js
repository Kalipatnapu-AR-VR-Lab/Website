...
const CustomMobileLink = ({ href, title, className = "", toggle }) => {
    ...
    return (
        <button href={href} className={`${className} relative group text-light my-2 tracking-widest uppercase`} onClick={handleClick}>
            {title}
            <span className={`h-[1px] inline-block bg-light absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 ${router.asPath === href ? 'w-full' : 'w-0'}`}>
                &nbsp;
            </span>
        </button>
    )
    ...
    return (
        <motion.header className={`w-full px-32 font-bold flex items-center justify-between z-20 lg:px-16 md:px-12 sm:px-8 border-b-[0.5px] lg:border-none border-dark/80 fixed top-0 duration-300  ${isShrunk ? "py-8" : "py-4 glassbg"}`} {...fadeAnimation}>
            ...
            <button className='flex-col items-center justify-cente hidden lg:flex z-40' onClick={handleClick}>
                ...
                <span className={` bg-dark duration-300 ease-out h-0.5 w-6 rounded-sm my-0.5 ${isOpen ? 'opacity-0' : 'opacity-100'}`}></span>
                <span className={` bg-dark duration-300 ease-out h-0.5 w-6 rounded-sm ${isOpen ? '-rotate-45 -translate-y-1 bg-light' : 'translate-y-0.5'}`}></span>
            </button>
            ...
            {/* mobile */}
            {
                isOpen ? <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className='min-w-[100vw] min-h-screen flex flex-col items-center justify-between fixed  z-90 bg-dark/90 backdrop-blur-md py-32 top-0 left-0'>
